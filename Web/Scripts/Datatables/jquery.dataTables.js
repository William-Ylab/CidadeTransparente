...
/* Extensão do Jquery DataTable, para utilizar em SORTS com palavras com acentos*/
jQuery.fn.dataTableExt.oSort['string-no-accents-asc'] = function (x, y) {
    x = removeAccents(x).toLowerCase();
    y = removeAccents(y).toLowerCase();
    return ((x < y) ? -1 : ((x > y) ? 1 : 0));
};

jQuery.fn.dataTableExt.oSort['string-no-accents-desc'] = function (x, y) {
    x = removeAccents(x).toLowerCase();
    y = removeAccents(y).toLowerCase();
    return ((x < y) ? 1 : ((x > y) ? -1 : 0));
};
...
/* Extensão do Jquery DataTable, para utilizar em SORTS com dinheiro */
jQuery.fn.dataTableExt.oSort['currency-without-R$-asc'] = function (x, y) {
    var currencyX = parseFloat(x.replace("R$ ", "").replace('.', '').replace(',', '.').replace(')', '').replace('(', ''));
    var currencyY = parseFloat(y.replace("R$ ", "").replace('.', '').replace(',', '.').replace(')', '').replace('(', ''));
    ...
    return ((currencyX < currencyY) ? -1 : ((currencyX > currencyY) ? 1 : 0));
};

jQuery.fn.dataTableExt.oSort['currency-without-R$-desc'] = function (x, y) {
    var currencyX = parseFloat(x.replace("R$ ", "").replace('.', '').replace(',', '.').replace(')', '').replace('(', ''));
    var currencyY = parseFloat(y.replace("R$ ", "").replace('.', '').replace(',', '.').replace(')', '').replace('(', ''));
...
/* Ext para utilizar em SORTS com datas */
jQuery.fn.dataTableExt.oSort['date-asc'] = function (x, y) {
    var splitX = x.split('/');
    var splitY = y.split('/');

    var dateX = new Date(splitX[2], splitX[1] - 1, splitX[0]);
    var dateY = new Date(splitY[2], splitY[1] - 1, splitY[0]);
